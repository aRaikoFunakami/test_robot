...
init(autoreset=True)
agent_executor = None  # 後で初期化
screenshot_tool = None
generate_locators = None
...
# --- プランナー ---
class VisionPlanner:
    """Vision APIに対応したプランナー"""
    def __init__(self):
        self.llm = ChatOpenAI(model="gpt-4o", temperature=0)
    
    async def ainvoke(self, locator: str, image_url: str, user_input: str) -> Plan:
        messages = [
            SystemMessage(content="""For the given objective, come up with a simple step by step plan. \
This plan should involve individual tasks, that if executed correctly will yield the correct answer. Do not add any superfluous steps. \
The result of the final step should be the final answer. Make sure that each step has all the information needed - do not skip steps.

Use the following locator information: """ + str(locator)),
        ...
        # with_structured_outputを使用してシステマティックにプランを生成
...
class VisionReplanner:
    """Vision APIに対応したリプランナー"""
    def __init__(self):
        self.llm = ChatOpenAI(model="gpt-4o", temperature=0)
    
    async def ainvoke(self, state: PlanExecute, locator: str, image_url: str) -> Act:
        messages = [
            SystemMessage(content="""For the given objective, come up with a simple step by step plan. \
This plan should involve individual tasks, that if executed correctly will yield the correct answer. Do not add any superfluous steps. \
The result of the final step should be the final answer. Make sure that each step has all the information needed - do not skip steps.
...
Your original plan was this: """ + str(state["plan"]) + """
...
async def generate_locator():
    global screenshot_tool, generate_locators
...
async def plan_step(state: PlanExecute):
    locator, image_url = await generate_locator()
    
    # Vision API対応の構造化プランナーを使用
    plan = await vision_planner.ainvoke(locator, image_url, state["input"])
...
async def replan_step(state: PlanExecute):
    ...
    # Vision API対応の構造化リプランナーを使用
    output = await vision_replanner.ainvoke(state, locator, image_url)
